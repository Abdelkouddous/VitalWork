...
import { FiKey, FiCheck, FiPlusCircle, FiArrowLeft } from "react-icons/fi";
...
const ConfirmAccountEmployer = () => {
  ...
  // Set OTP from query params if available
  useEffect(() => {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      toast.success("Clinic account verified successfully!");
      navigate("/dashboard");
    } catch (error) {
      const message = error?.response?.data?.msg || error?.response?.data?.message || "Invalid verification code";
      toast.error(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Wrapper>
      <div className="form" style={{ maxWidth: "460px", width: "100%", textAlign: "center" }}>
        ...
        >
          <FiPlusCircle style={{ fontSize: "24px", color: "white" }} />
        </div>
        ...
        >
          Confirm Clinic Account
        </h1>
        <p style={{ color: "var(--text-secondary-color)", marginBottom: "1.5rem", fontSize: "0.875rem" }}>
          Enter the 6-digit confirmation code issued for your medical facility
        </p>
        ...
        >
          ...
          <p className="text-xs text-[var(--text-secondary-color)] mb-2.5">
            Your clinic verification code is preserved in the label below without requiring external email lookup:
          </p>
        ...
        <form onSubmit={handleVerify}>
          ...
          >
            {loading ? "Verifying..." : "Verify & Access Clinic Dashboard"}
          </button>
        </form>
